...
test -z "$JAVA_HOME"             && export JAVA_HOME=/opt/jdk
...
test -z "$ACCUMULO_TSERVER_OPTS" && export ACCUMULO_TSERVER_OPTS="${POLICY} -Xms2g -Xmx${TSERVER_XMX:-3g} -XX:NewSize=1G -XX:MaxNewSize=1G"
test -z "$ACCUMULO_MASTER_OPTS"  && export ACCUMULO_MASTER_OPTS="${POLICY} -Xms1g -Xmx${MASTER_XMX:-2g}"
test -z "$ACCUMULO_MONITOR_OPTS" && export ACCUMULO_MONITOR_OPTS="${POLICY} -Xms256m -Xmx${MONITOR_XMX:-1g}"
test -z "$ACCUMULO_GC_OPTS"      && export ACCUMULO_GC_OPTS="-Xms512m -Xmx${GC_XMX:-1g}"
...
test -z "$ACCUMULO_OTHER_OPTS"   && export ACCUMULO_OTHER_OPTS="-Xms256m -Xmx1g"
